...
export function createRelay(config) {
  ...
  async function arm(req, res) {
    ...
    if (j.promptLightDurationMs != null) session.promptLightDurationMs = j.promptLightDurationMs;
    if (j.promptEndsAt != null) session.promptEndsAt = j.promptEndsAt;
    persist();
    sendJson(res, 200, {
      ok: true,
      sid: session.sid,
  ...
  async function challengeStart(req, res) {
    ...
    session.promptLightDurationMs = j.promptLightDurationMs ?? session.promptLightDurationMs ?? null;
    ...
    session.transitionToleranceMs = j.transitionToleranceMs ?? j.phaseToleranceMs ?? session.transitionToleranceMs ?? 250;
